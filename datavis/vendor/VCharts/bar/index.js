 ...
 * @Last Modified time: 2018-04-26 17:52:36
 */
...
class Bar {
  ...
  validate(options = []) {
    ...
    options.forEach((option, index) => {
      tmpOption[index] = self.validateOption(option, index, options.length);
    });
    myChart.option.series = myChart.option.series || {};
    myChart.option.series.bar = tmpOption;
    self.countBarGapData();
  }
  ...
  validateOption(option, index, length) {
    ...
    if (option.name === undefined) {
      tmpOption.name = `bar-${index}`;
    }

    // 补全option缺失的值
    myChart.util.ObjectsDeepAssign(true, currentOption, myChart.theme.bar || {}, tmpOption);
  ...
  countBarGapData() {
    ...
    myChart.option.series.bar.forEach((option) => {
      self.countBarPosition(option, 'xAxis');
      self.countBarPosition(option, 'yAxis');
    });
  }
  ...
  static countBarStarts(barPosition) {
    ...
    Object.keys(barPosition.barWidths).forEach((barWidth, index) => {
      ...
      if (index === 0) {
        ...
        const preBarStart = barStarts[index - 1];
      ...
      barStarts.push(currentBarStart);
    });
  ...
  initBars() {
    ...
    myChart.option.series.bar.forEach((option) => {
      ...
      self.initSingleBar(option, axisIndex[option.xAxisIndex]);
  ...
  initSingleBar(option, index) {
    ...
    const barItem = myChart.grid
      ...
      .classed('last', false);
    const barsAttribute = self.countBarsAttribute(option, index);
    ...
    if (option.markPoint.show === true) {
      ...
      barMarkPoints
        ...
        .attr('x', (d) => {
          const markPointX =
            (d.x - ((option.markPoint.symbolSize[0] - d.width) / 2))
            + option.markPoint.symbolOffset[0];
          return markPointX;
        })
        .attr('y', (d) => {
   ...
   * @memberof Bar
   */
  countBarsAttribute(option, index) {
    const self = this;
    const { myChart } = self;
    ...
    const barsAttribute = option.data.map((d, i, data) => {
      ...
      if (!Object.prototype.hasOwnProperty.call(barPosition, 'barStarts')) {
        Bar.countBarStarts(barPosition);
      }
      // 偏移长度
