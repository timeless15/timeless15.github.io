 ...
 * @Last Modified time: 2018-04-24 17:35:23
 */
...
class Line {
  ...
  validate(options = []) {
    ...
    options.forEach((option, index) => {
      tmpOption[index] = self.validateOption(option, index);
    });
    myChart.option.series = myChart.option.series || {};
    myChart.option.series.line = tmpOption;
  }
  ...
  validateOption(option, index) {
    ...
    const defaultOption = {
      ...
      lineStyle: {
        normal: {
          ...
          color: myChart.util.defaultColor(index, myChart.theme.color),
          // color渐变色
          // color: {
          //   type: 'linear',
          //   x: '0%',
    ...
    if (option.name === undefined) {
      tmpOption.name = `line-${index}`;
    }

    // 补全option缺失的值
    myChart.util.ObjectsDeepAssign(true, currentOption, myChart.theme.line || {}, tmpOption);
    ...
    if (myChart.util.isObject(currentOption.lineStyle.normal.color)) {
      ...
      = myChart.util.initLinearGradient(currentOption.lineStyle.normal.color);
    }
    ...
    if (myChart.util.isObject(currentOption.itemStyle.normal.color)) {
      ...
      = myChart.util.initLinearGradient(currentOption.itemStyle.normal.color);
    }

    // 生成渐变色
    if (myChart.util.isObject(currentOption.areaStyle.normal.color)) {
      currentOption.areaStyle.normal.color
      = myChart.util.initLinearGradient(currentOption.areaStyle.normal.color);
    }
  ...
  init() {
    ...
    myChart.grid
      ...
      .append('g')
      .classed('lines', true);
    myChart.option.series.line.forEach((lineOpt, lineIndex) => {
      self.initSingleLine(lineOpt, lineIndex);
    });
  ...
  initSingleLine(lineOpt, lineIndex) {
    ...
    const yPos = d => myChart.yScale[lineOpt.yAxisIndex](d);
    ...
    line
      ...
      .attr('d', lineBuilder)
      .style('stroke-dasharray', () => (this.getTotalLength !== undefined ? this.getTotalLength() : 2000))
      .style('stroke-dashoffset', () => (this.getTotalLength !== undefined ? this.getTotalLength() : 2000))
      .transition()
      .duration(myChart.util.config.transitionDurationTime)
      .ease(d3.easeExpInOut)
      .style('stroke-dashoffset', 0)
      .style('z-index', lineIndex);
